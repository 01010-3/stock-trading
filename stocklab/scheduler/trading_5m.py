...
def check_buy_completed_order(code):
    """매수완료된 주문은 매도 주문"""
    buy_completed_order_list = list(mongo.find_items({"$and":[{"code":code},
...
def check_sell_order(code):
    # 매도주문 완료 체크
    sell_order_lsit = list(mongo.find_items({"$and": [{"code": code},
                                                     {"status": "sell_ordered"}]},
                                                     "stocklab_demo", "order"))
    for order in sell_order_lsit:
        time.sleep(1)
        code = order["code"]
